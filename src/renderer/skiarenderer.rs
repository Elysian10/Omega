...
impl SkiaRenderer {
    ...
    pub fn render(dom: &mut Dom, buffer: &mut [u32], width: usize, height: usize, debug_tools: Option<&mut DebugTools>, event_system: Option<&mut EventSystem>) {
        ...
        let bytes = unsafe { std::slice::from_raw_parts_mut(buffer.as_mut_ptr() as *mut u8, height * stride) };
        ...
        if let Some(mut skia_surface) = surfaces::wrap_pixels(&info, bytes, stride, None) {
            ...
            if let Some(debug_tools) = debug_tools {
                ...
                // Draw debug tools
                debug_tools.draw_debug_tools(&dom, &canvas, event_system.unwrap(), dom_width as f32, 0.0, dom_width as f32, height as f32);
            }
        }
    }
    ...
    fn draw_text(canvas: &Canvas, text: &Text, style: &ComputedTextStyle, layout_data: LayoutData, text_info: &TextInfo) {
        ...
        for (i, line) in text_info.lines.iter().enumerate() {
            ...
            canvas.draw_str(line, Point::new(content_x, baseline_y), &font, &paint);
