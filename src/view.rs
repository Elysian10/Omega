...
pub fn create_view(dom: &mut Dom, parent_id: NodeId){
    ...
    dom.try_append_child(parent_id, child_node_id).unwrap();

    let child2 = Element::new(Color::new(1.0, 0.0, 1.0, 0.3));
    let child2_node_id = dom.create_element(child2);
    dom.try_append_child(parent_id, child2_node_id).unwrap();

    let child3 = Text::new("test\nme", Color::new(1.0, 1.0, 1.0, 1.0));

    let child3_node_id = dom.create_text(child3);

    dom.try_append_child(parent_id, child3_node_id).unwrap();
