// /src/dom/debug_toools

use crate::dom::{Dom, dom::NodeId, events::EventSystem, fontmanager::get_thread_local_font_mgr};
use skia_safe::{Canvas, Color4f, Font, FontStyle, Paint, Point};
...
impl DebugTools {
    ...
    pub fn draw_debug_tools(&self, dom: &Dom, canvas: &Canvas, event_system: &EventSystem, x: f32, y: f32, width: f32, height: f32) {
        // Use the thread-local font manager
        let font_mgr = get_thread_local_font_mgr();
        let typeface = font_mgr.match_family_style("Arial", FontStyle::normal()).unwrap_or_else(|| font_mgr.legacy_make_typeface(None, FontStyle::normal()).unwrap());
    ...
    fn draw_json_string(canvas: &Canvas, json_string: String, start_x: f32, start_y: f32, font: &Font, paint: &Paint) {
        let mut y_offset = start_y;
        let line_height = 18.0;

        // Split the JSON string by lines
        for line in json_string.lines() {
            // Remove any escaped quotes for cleaner display
            let clean_line = line.replace("\\\"", "\"");//.replace("\\n", "");

            // Draw the line
            canvas.draw_str(&clean_line, Point::new(start_x, y_offset), font, paint);
